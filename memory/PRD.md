...
- 2026-05-13: **Catalogue activités gérable depuis le dashboard** — CRUD `POST/PATCH/DELETE /api/staff/activities` ouvert aux rôles `manager+admin` (au lieu de `admin` seul). Nouvelle page `/staff/configuration/activites` (`StaffActivitiesConfig.jsx`) : table des activités existantes (nom, catégorie, prix FCFA, durée, statut actif), modale CRUD avec champs nom FR/EN, description FR/EN, catégorie (Activité/Bien-être/Excursion/Restauration/Autre/Nautique/Terrestre), prix, durée, identifiant auto-slugifié, toggle actif/désactivé. Item de menu sidebar "Catalogue activités" (manager+admin). Les ajouts apparaissent immédiatement dans la grille du scanner. Validé : 9 activités existantes affichées, création "Pédalo 30 min · 12 000 FCFA" OK, toast confirmation.
