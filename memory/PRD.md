...
- 2026-02-14: **Consommation sur place dans le pôle Activités & Événements** — `GET /staff/poles/activites_events/overview` retourne désormais un objet `wallet_stats` (KPIs charges actives/CA encaissé/annulées + panier moyen, by_category, top_items, daily_trend) agrégeant les 30 derniers jours de transactions wallet. Frontend `StaffPolePage.jsx` : nouveau composant `WalletStats` (rendu uniquement pour `activites_events`) avec 4 tuiles KPI, AreaChart tendance quotidienne, barres répartition catégories, top 5 consommations, bannière d'alerte des charges annulées + deep-link vers `/staff/consommation`. Validé via curl (active=20 / revenue=778k / voided=5) + screenshot Playwright (tous data-testid présents).
