...
- 2026-05-13: **QR billet = carte universelle (embarquement + activités + consommations)** — Le QR ticket sert désormais aussi à charger les activités et consommations sur place, avec **traçabilité par participant**. Backend : `WalletCharge` étendu avec `participant_token` (résolu en `participant_name = "Surname Name"` du QR scanné) → stocké sur chaque transaction + propagé au reçu fiscal. Nouvel endpoint `POST /api/staff/scan/{qr_token}/charge` : raccourci qui scanne le ticket, retrouve le wallet du booking, ajoute la charge taggée du `participant_token` et émet un reçu nominatif. `GET /api/staff/scan/{token}` enrichi avec `participant_charges`, `participant_total_charged`, `wallet_status`, `wallet_total_charged`. UI scanner : nouveau panneau "Activités & consommations" affiché juste après le scan ticket → grille de 9 boutons catalogue (Jet Ski, Quad, Paddle, Spa, Excursion…) cliquables, champ libellé+montant libre pour consommations (Cocktail, snacks), historique des charges du participant + total individuel. UI page Activités : ligne sous chaque label montrant "Pour {Surname Name}" pour identifier qui a consommé. Validé en e2e : 2 participants Marc/Sara du même booking → chacun a son total isolé (49k vs 35k), wallet total 84k.
