...
- 2026-05-13: **Capacités paramétrables par salle (Le Kaai)** — Nouvelle collection `kaai_zones` avec `{name, capacity, sort_order}`. Seedé au boot : Terrasse 1 (24), Terrasse 2 (24), Salle (32). Migration auto des tables existantes : les anciennes "Terrasse" sont réparties moitié-moitié entre Terrasse 1 et Terrasse 2 ; zones inconnues → "Salle". Endpoints `GET/POST/PATCH/DELETE /api/staff/kaai/zones` (manager+admin pour PATCH, admin seul pour create/delete). `GET /api/staff/kaai/day` enrichi : ajout du tableau `zones` avec `used / available / saturation_pct`. Guard d'overbooking activé sur `PATCH /api/staff/kaai/bookings/{id}/table` : refuse l'assignation si le total de couverts (adults + children) déjà placés dans la zone + ceux de cette réservation dépasse la capacité (HTTP 400 explicite "Capacité de la salle « X » dépassée (Y/Z couverts pour le DATE)."). UI : section "Capacités des salles" sur `/staff/kaai` avec inputs éditables manager+admin, barres de saturation colorées (or → ambre 90% → rouge 100%), select de zone dans la création de table piloté par la collection.
