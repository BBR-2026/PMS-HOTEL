...
- 2026-05-13: **Module Reçus de paiement signés** — Nouvelle collection `receipts` + génération automatique de reçus fiscaux à chaque transaction monétaire : (1) charge sur wallet activités → 1 reçu par charge avec `source="activity"`, (2) paiement de réservation public ou staff → `source="booking"`, (3) nouveau endpoint `POST /api/staff/loisirs/events/{event_id}/payment` pour les privatisations → `source="event"`. Chaque reçu porte un numéro séquentiel journalier `BBR-YYYYMMDD-XXXXX` (compteur atomique MongoDB) et une signature HMAC-SHA256 16 caractères (`RECEIPT_SECRET` ou fallback `JWT_SECRET`). Endpoints `GET /api/staff/receipts` (filtres source/date_from/date_to/q/payment_method + pagination + summary_by_source), `GET /api/staff/receipts/{id}` (JSON détaillé) et `GET /api/staff/receipts/{id}.pdf` (PDF reportlab BBR stylisé avec entête, métas, lignes, totaux, signature numérique, footer). Page `/staff/recus` (`StaffReceipts.jsx`) avec KPIs total/activités/réservations/événements, filtres date+source+recherche, table paginée, modale détail (signature visible) et téléchargement PDF. Accès restreint manager+admin (réception 403). Validé : 4 reçus créés, PDF 2950 octets, signature HMAC vérifiée, modale + screenshot OK.
