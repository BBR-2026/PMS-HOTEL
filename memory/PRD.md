...
- 2026-05-13: **Traçabilité enrichie des scans (staff + bateau)** — Chaque check-in `POST /staff/scan/{token}/checkin` enregistre désormais `staff_name`, `boat_time`, `boat_date`, `boat_label` (ex. "12H aller", "16H retour") — auto-résolus depuis la réservation (boat_time pour aller, return_boat_time pour retour). Le toast UI confirme "Embarquement aller enregistré · Bateau 12H aller". Sur la fiche scanner, chaque ligne d'historique de scan affiche désormais un badge or "Bateau 12H aller" + "par {Manager}". Les charges activités/consommations affichent "par {staff_name}" sous chaque ligne. Nouveau endpoint `GET /api/staff/checkins/history` (manager+admin) : pipeline MongoDB qui agrège tous les scans de tous les bookings, filtres `date`, `date_from/to`, `boat_time`, `direction`, `q` (texte libre client/staff) + pagination + summary par bateau. Nouvelle page `/staff/embarquements-historique` (`StaffCheckinsHistory.jsx`) avec cartes résumé par bateau, filtres complets, table date/heure/client/sens/bateau/offre/staff. Validé e2e : 3 scans tracés correctement, summary "12H aller: 2, 12H retour: 1".
