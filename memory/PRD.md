...
- 2026-05-12: **Embarquement aller/retour 2 scans max** — Nouveau endpoint `POST /api/staff/scan/{qr_token}/checkin` qui enregistre un scan d'embarquement (max 2 par QR : 1er=aller, 2e=retour, 3e=400). Le statut booking évolue automatiquement : `confirmed` → `arrived` (tous les QR ont 1 scan) → `completed` (tous ont 2 scans). Chaque scan persiste `direction`, `scanned_at`, `staff_email` dans `qr_codes[].scans`. UI scanner : bouton dynamique "Embarquement aller" puis "Embarquement retour" puis bandeau "QR entièrement utilisé"; historique des scans avec horodatage FR + email du staff. Dates affichées au format `JJ/MM/AAAA`. Bouton "Scanner un autre QR" fixé : reset attend la libération de la caméra avant de remonter la zone DOM (`requestAnimationFrame` x2 + check `getElementById`) puis le useEffect relance la caméra proprement.
